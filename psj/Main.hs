...
compile (PCCOptions input _ outputDir opts usePrefix) = do
  ...
  case runWriterT (parseInputs moduleFiles []) of
    ...
    Right ((ms, _), warnings) -> do
      ...
      exitFailure
--          makeActions = P.buildMakeActions outputDir filePathMap usePrefix
{-
      e <- P.runMake opts $ P.make makeActions (map snd ms)
      case e of
        Left errs -> do
...
main = execParser opts >>= compile
  ...
  headerInfo  = header   "psj - Compiles PureScript to Java"
  footerInfo  = footer $ "psj " ++ showVersion Paths.version
